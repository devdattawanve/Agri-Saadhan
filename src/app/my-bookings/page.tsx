            ...
            <CardContent className="space-y-4">
                ...
                {bookings && bookings.length > 0 && bookings.map(booking => (
                    <div key={booking.id} className="border rounded-lg p-4 flex justify-between items-center">
                        <div>
                            <p className="font-semibold">Booking ID: <span className="font-mono text-sm">{booking.id.substring(0, 7)}...</span></p>
                            <p className="text-sm text-muted-foreground">
                                Equipment ID: <span className="font-mono text-sm">{booking.equipmentId.substring(0, 7)}...</span>
                            </p>
                            <p className="text-sm text-muted-foreground">
                                Booked on: {booking.createdAt?.toDate ? format(booking.createdAt.toDate(), 'PPP') : 'Just now'}
                            </p>
                        </div>
                        <div className="text-right">
                           <Badge variant={booking.status === 'completed' ? 'default' : 'secondary'} className="capitalize">{booking.status}</Badge>
